...
if (UnderAttackThreat() > 50)and(s_selfTotalValue < s_enemyTotalValue) then
...
attack_now_timer = function() 
	...
	-- 
	AttackNow();
	if (UnderAttackThreat() < -50) and (s_selfTotalValue > s_enemyTotalValue)and(gameTime() > lastAttackTime + 60)then
		Research( FXP_ATTACK )
