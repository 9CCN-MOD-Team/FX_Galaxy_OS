...
function findAndAttackEnemyMissiles(CustomGroup, playerID, shipID)
    ...
    if (Selection_GetMissiles("AllMissiles") > 0) then
        ...
        --...should not attack allied missiles either
       	--Selection_FilterExclude("AllMissiles", "AllMissiles", "AutoformationFamily", "Mine", "")
       	Selection_FilterExclude("AllMissiles", "AllMissiles", "PlayerOwner", ""..playerID, "")
        for iPlayer=0,Universe_PlayerCount()-1 do
        	if (iPlayer~=playerID) then
        		if (AreAllied(iPlayer, playerID)==1) then
        			Selection_FilterExclude("AllMissiles", "AllMissiles", "PlayerOwner", ""..iPlayer, "")
        		end
        	end
        end
        --if (Selection_FilterExclude("IncomingMissiles", "AllMissiles", "PlayerOwner", ""..playerID, "") > 0) then
            ...
            numberEnemyMissiles = Selection_FilterInclude("IncomingMissiles", "AllMissiles", "NearPoint", defensePointString, ""..1500)
...
function updateDefenseFighterSelfDefense(CustomGroup, playerID, shipID)
    local Attackers = "defenseFighterGuardTargetAttackers"
    SobGroup_CreateIfNotExist(Attackers)
    Selection_Create("AllMissiles")
    if (Selection_GetMissiles("AllMissiles") > 0) then
        SobGroup_GetAttackers(CustomGroup, Attackers)
        if (SobGroup_Count(Attackers) > 0) then
            local enemyPlayerID = SobGroup_GetPlayerOwner(Attackers);
            Selection_Create("IncomingMissiles")
            if (Selection_FilterInclude("IncomingMissiles", "AllMissiles", "PlayerOwner", ""..enemyPlayerID, "") > 0) then
                SobGroup_AttackSelection(playerID, CustomGroup, "IncomingMissiles", 1)
            end
        else
        	findAndAttackEnemyMissiles(CustomGroup, playerID, shipID)
        end
    end
end
--
...
function updateDefenseFighter(CustomGroup, playerID, shipID)
    ...
    --else
    --    updateDefenseFighterSelfDefense(CustomGroup, playerID, shipID)
    --end
