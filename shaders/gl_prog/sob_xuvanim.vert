...
void main()
{
	...
	if(inUVAnimParams.z!=0)//rotation
	{
		int rDir = int(inUVAnimParams.w);

		float rotationAngle = (inTime.x * inUVAnimParams.z);
		mat2 UVRotationMatrix;
	
		if(rDir==0)
		{
			UVRotationMatrix = mat2( cos(rotationAngle), sin(rotationAngle),-sin(rotationAngle), cos(rotationAngle));
			//CW
		}
		else
		{
	...
	float moveY=0.0;
	// calculate move dis
	if(inUVAnimParams.x!=0)
	{
		moveX= (inTime.x / inUVAnimParams.x);
	}
	if(inUVAnimParams.y!=0)
	{	
		moveY= (inTime.x / inUVAnimParams.y);
	}

	outUV0.x = outUV0.x + moveX;
	outUV0.y = outUV0.y + moveY;
