...
uniform sampler2D inTexMeta;
uniform sampler2D inTexGloA;
uniform sampler2D inTexGloB;
uniform sampler2D inTexGloC;
uniform sampler2D inTexGloD;

uniform vec4 inFlashParams;
uniform vec4 inFlashAddParams;
...
void main()
{
	...
	//vec3 worldNorm = normalize(toWorld*texNorm);
#endif
	...
	texGlow.b = mix(texGlow.b, 0.5, texBadge.a*0.35);
...
if( inFlashAddParams.b==1)
{
...
#ifdef UX_DEBUG
	//extSpecPower=4.5;//高光乘方（大概） ；比率  注意，高光和DIFF是mix的 
	extSpecPower=4.5*texture(inTexMeta,outUV0).g;
#endif
	...
	extSpecPower = mix(extSpecPower, extSpecPower*extPaintDim, isPaint);//spec Power混合
	
	float specGloss = max(1.0, pow(mapSpecSharp, extGlossCurve)*extGlossScale)+extGlossBias;

#ifdef UX_DEBUG
	specGloss=500*texture(inTexMeta,outUV0).r;//主光源Gloss
#endif
...
//	extFrenPower=1000;//菲涅耳度，越大，反射角度、越大
	...
	// Map Channel * Powers
	float glowStrength = clamp((mapGlowMask*extGlowPower)-(extGlowFren*frenDeg), 0.0, 1.0);
...
//	reflStrength+=1;
	...
	float diffStrength = max(0.0, 1.0-(specStrength+(mapGlowMask*extGlowPower)));//剩下的是Diff

#ifdef UX_DEBUG
	
	diffStrength = max(0.0, 1.0-( texture(inTexMeta,outUV0).b*specStrength+(mapGlowMask*extGlowPower)) );//按照Metal Map与Spec混合
//	diffStrength = max(0.0, 1.0-(mapGlowMask*extGlowPower));//不与Spec混合
	...
	float litSpec = clamp(dot(-worldEye, litRefl), 0.0, 1.0);
	...
	litSpec = clamp(dot(-worldEye, litRefl), 0.0, 1.0);
	litSpec = pow(litSpec, specGloss)*min(litDif*1000, 1.0);
	...
	for (; inL<inP; inL++)
	{
		...
		float subSpec = clamp(dot(-worldEye, reflect(-litVec, worldNorm)), 0.0, 1.0);
	...
	for (; inL<inB; inL++)
	{
		...
		subSpec = min(subSpec*(1.0+(subBoost*0.25)), 1.0);
	...
	diffAll = (diffAll*vec3(diffStrength))+vec3(glowStrength);//赋予Glow，自发光

	float total_time=inFlashParams.r;
	float threshold=inFlashParams.g;
	float is_flashing=inFlashParams.b;
	float use_tex_blend=inFlashParams.a;

	float int_max=inFlashAddParams.r;
	
	float time_now=mod(inTime.x,total_time);//0-total_time

	if(is_flashing==1)
	{	
		//flash always blend;
		if( abs(time_now-0)<threshold )
		{
			float A_ratio=1-(abs(time_now-0)/threshold);
			diffAll+=A_ratio*texture(inTexGloA,outUV0).rgb;
		}

		if( abs(time_now-0.25*total_time)<threshold )
		{
			float B_ratio=1-(abs(time_now-0.25*total_time)/threshold);
			diffAll+=B_ratio*texture(inTexGloB,outUV0).rgb*int_max;
		}

		if( abs(time_now-0.5*total_time)<threshold )
		{
			float C_ratio=1-(abs(time_now-0.5*total_time)/threshold);
			diffAll+=C_ratio*texture(inTexGloC,outUV0).rgb*int_max;
		}

		if( abs(time_now-0.75*total_time)<threshold )
		{
			float D_ratio=1-(abs(time_now-0.75*total_time)/threshold);
			diffAll+=D_ratio*texture(inTexGloD,outUV0).rgb*int_max;
		}


		if(abs( time_now-total_time)<threshold)
		{
			float A2_ratio=1-(abs(time_now-total_time)/threshold);
			diffAll+=A2_ratio*texture(inTexGloA,outUV0).rgb*int_max;
		}
	}
	...
	{
	//blend tex
	float A_ratio=0,B_ratio=0,C_ratio=0,D_ratio=0,A2_ratio=0;
	float b_threshold=total_time/4;
		if( abs(time_now-0)<b_threshold )
		{
			A_ratio=1-(abs(time_now-0)/b_threshold);
		}

		if( abs(time_now-0.25*total_time)<b_threshold )
		{
			B_ratio=1-(abs(time_now-0.25*total_time)/b_threshold);
		}

		if( abs(time_now-0.5*total_time)<b_threshold )
		{
			C_ratio=1-(abs(time_now-0.5*total_time)/b_threshold);
		}

		if( abs(time_now-0.75*total_time)<b_threshold )
		{
			D_ratio=1-(abs(time_now-0.75*total_time)/b_threshold);
		}
		if(abs( time_now-total_time)<b_threshold)
		{
			A2_ratio=1-(abs(time_now-total_time)/b_threshold);
		}

	diffAll+=A_ratio*texture(inTexGloA,outUV0).rgb;
	diffAll+=B_ratio*texture(inTexGloB,outUV0).rgb;
	diffAll+=C_ratio*texture(inTexGloC,outUV0).rgb;
	diffAll+=D_ratio*texture(inTexGloD,outUV0).rgb;
	diffAll+=A2_ratio*texture(inTexGloA,outUV0).rgb;
