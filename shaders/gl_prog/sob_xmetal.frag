...
#define UX_DEBUG
...
#ifdef UX_DEBUG
	//extSpecPower=4.5;//高光乘方（大概） ；比率  注意，高光和DIFF是mix的  (EDIT:now we don't mix DIFF WITH SPEC 
	...
	specGloss=clamp(specGloss,5,500);
...
//	reflStrength+=1;
	...
	diffStrength = max(0.0, texture(inTexMeta,outUV0).b+(mapGlowMask*extGlowPower));//按照Metal Map与Spec混合
