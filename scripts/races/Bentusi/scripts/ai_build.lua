kInterceptor = BEN_FIGHTER
kCarrier = BEN_FLOATINGCITY
kShipYard = BEN_BENTUS
kDestroyer = BEN_DESTROYER
kBattleCruiser = BEN_BATTLESHIP

function DetermineDemandWithNoCounterInfo_Bentusi()
...
function DetermineSpecialDemand_Bentusi()
	...
	if (s_selfTotalValue > 120) then
		...
		if (s_race == Race_Bentusi ) then
			local capscount = NumSquadrons(BEN_FLOATINGCITY)
			if (capscount > 0) then
...
Proc_DetermineDemandWithNoCounterInfo = DetermineDemandWithNoCounterInfo_Bentusi
Proc_DetermineSpecialDemand = DetermineSpecialDemand_Bentusi