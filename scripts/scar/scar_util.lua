...
function HW2FX_EventStartSet(iCameraControl,iCameraZoom,iMuteActor,iAllSpeech,iEnableSkip,iLetterBox)
	local iTable=
	{ 
            { "Camera_AllowControl("..iCameraControl..")", "", }, 
            { "Sensors_EnableCameraZoom( "..iCameraZoom.." )", "", }, 
	        { "Sound_SetMuteActor('"..iMuteActor.."')", "", }, 
	        { "Sound_EnableAllSpeech( "..iAllSpeech.." )", "", }, 
            { "Universe_EnableSkip("..iEnableSkip..")", "", }, 
	}
	if (iLetterBox==1) then
		iTable[6]={ "Sound_EnterIntelEvent()", "", }
		iTable[7]=HW2_Letterbox(1)
		iTable[8]=HW2_Wait(2)
	end
	return iTable
end

function HW2FX_EventEndSet(iCameraControl,iCameraZoom,iAllSpeech,iEnableSkip,iLetterBox)
	local iTable=
	{ 
            { "Camera_AllowControl("..iCameraControl..")", "", }, 
            { "Sensors_EnableCameraZoom( "..iCameraZoom.." )", "", }, 
	...
	if (iLetterBox==1) then
		iTable[5]={ "Sound_ExitIntelEvent()", "", }
		iTable[6]=HW2_Letterbox(0)
		iTable[7]=HW2_Wait(2)
	end
	return iTable
end

function HW2_SubTitleEvent2(actor, text, time)
        local newTable = {}
        newTable[1] = "Subtitle_Add2("..actor..",\""..text.."\","..time..")";
        newTable[2] = "Subtitle_IsDone2("..actor..")";
        return newTable;
end

function Subtitle_Add2(actor,text,time)
	if time==nil then
		Subtitle_Showing[actor]=Universe_GameTime()+floor((strlen(text))/4)/2
	else
		Subtitle_Showing[actor]=Universe_GameTime()+time
	end
	UI_SetElementSize("SubtitleScreen2","Region1_Icon"..actor,100,50)
	UI_SetElementVisible("SubtitleScreen2","Region1_Icon"..actor,1)
	UI_SetElementVisible("SubtitleScreen2","Region2_Text",0)
	UI_SetTextLabelText("SubtitleScreen2","Region1_Text",text)
	UI_ShowScreen("SubtitleScreen2",ePopup)
	if(Rule_Exists("Subtitle_Check")==0)then
		Rule_Add("Subtitle_Check")
	end
end

function Subtitle_Add3(actor,text1,text2,time)
	local iText=text1..text2
	if time==nil then
		Subtitle_Showing[actor]=Universe_GameTime()+floor((strlen(iText))/4)/2
	else
		Subtitle_Showing[actor]=Universe_GameTime()+time
	end
	UI_SetElementSize("SubtitleScreen2","Region1_Icon"..actor,100,50)
	UI_SetElementVisible("SubtitleScreen2","Region1_Icon"..actor,1)
	UI_SetElementVisible("SubtitleScreen2","Region2_Text",1)
	UI_SetTextLabelText("SubtitleScreen2","Region1_Text",text1)
	UI_SetTextLabelText("SubtitleScreen2","Region2_Text",text2)
	UI_ShowScreen("SubtitleScreen2",ePopup)
	if(Rule_Exists("Subtitle_Check")==0)then
		Rule_Add("Subtitle_Check")
	end
...
function Subtitle_Add4(actor,text1,text2,text3,time)
	local iText=text1..text3
	if time==nil then
		Subtitle_Showing[actor]=Universe_GameTime()+floor((strlen(iText))/4)/2
	else
		Subtitle_Showing[actor]=Universe_GameTime()+time
	...
	UI_SetElementVisible("SubtitleScreen2","Region1_Icon"..actor,1)
	UI_SetElementVisible("SubtitleScreen2","Region2_Text",1)
	UI_SetTextLabelTextWF1("SubtitleScreen2","Region1_Text",text1,text2)
	UI_SetTextLabelText("SubtitleScreen2","Region2_Text",text3)
	UI_ShowScreen("SubtitleScreen2",ePopup)
	if(Rule_Exists("Subtitle_Check")==0)then
		Rule_Add("Subtitle_Check")
	end
...
function Subtitle_Check()
	local iClose=1
	for actor,time in Subtitle_Showing do
		if Universe_GameTime()>Subtitle_Showing[actor] then
			Subtitle_Showing[actor]=nil
			UI_SetElementSize("SubtitleScreen2","Region1_Icon"..actor,0,0)
			UI_SetElementVisible("SubtitleScreen2","Region1_Icon"..actor,0)
			UI_SetTextLabelText("SubtitleScreen2","Region1_Text","")
			UI_HideScreen("SubtitleScreen2")
		else
		end
		iClose=0
	end
	if(iClose==1)then
		Rule_Remove("Subtitle_Check")
	end
end

function Subtitle_IsDone2(actor)
	if Subtitle_Showing[actor]~=nil then
		if Universe_GameTime()>Subtitle_Showing[actor] then
			return 1
		else
			return 0
...
function HW2_Letterbox2( state )
	local newTable = {}
	newTable[1] = "Camera_SetLetterboxState("..state..", 2 )";
	newTable[2] = "";
	return newTable;
end
