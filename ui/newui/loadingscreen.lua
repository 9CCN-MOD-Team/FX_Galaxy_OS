function FX_LoadingBG()
	dofilepath("player:DTM4State.lua")
	local FXBG = 
			{
				size = { 0, 0 },
				texture = "DATA:/UI/NewUI/Background/loading.dds",
				textureUV = { 0, 0, 1920, 1080},
			} 
	if(DTM4_State~=nil)and(DTM4_State~="")then
		FXBG = 
			{
				size = { 0, 0 },
				texture = "DATA:/leveldata/campaign/tutorial/RPG/LoadingBG/"..DTM4_State..".dds",
				textureUV = { 0, 0, 1920, 1080},
			} 
	end
	return FXBG
...
function FX_LoadingTips()
		dofilepath("locale:LoadingTips.lua")
		local TipIndex=1
		while LoadingTips[TipIndex]~=nil do
			TipIndex=TipIndex+1
		end
		TipIndex=random(TipIndex-1)
...
LoadingScreen = {
	...
	-- Aspect Enforcement Frame
	{
		...
		-- Per-Level Dynamic Image 
		{
			...
			name = "bgImage",
			BackgroundGraphic = FX_LoadingBG(),
		},
		...
		},
		{
			...
			Layout = {
				sector = 1,
				pos_XY = { x = 0, y = 0.94, xr = "par", yr = "par" },
				size_WH = { w = 1, h = 0.06, wr = "par", hr = "par" },
			},
